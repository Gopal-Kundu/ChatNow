    ...
    <div className="bg-transparent max-h-screen border border-white   flex flex-col">
        ...
        <div className='py-4 border border-white bg-white/40 flex items-center justify-center gap-3'>
            <div className='relative'>
                <input type="text" placeholder='Search Conversations' className='border border-white focus:bg-white bg-white/50 rounded-xl ml-1 pl-9 w-70 outline-none py-1' value={search} onChange={(e)=>setSearch(e.target.value)}/>
        ...
        <div className='webkit-scrollbar overflow-y-auto flex-1'>   
            {chat?.filter(perChat => perChat.username.toLowerCase().includes(search.toLowerCase()) )?.map((perChat,idx)=>{
